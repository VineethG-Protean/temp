...
const defineStepper = <const Steps extends Stepperize.Step[]>(
  ...
  const StepperContainer = ({
    ...
    return (
      <div className={cn("stepper ptn-w-full", className)} {...props}>
  ...
  return {
    ...
    StepperStep: ({ children, className, icon, ...props }) => {
      ...
      if (variant === "circle") {
        return (
          <li
            className={cn(
              "stepper-step ptn-flex ptn-shrink-0 ptn-items-center ptn-gap-4 ptn-rounded-md ptn-transition-colors",
              className
            )}
          >
            <CircleStepIndicator
            ...
            />
            <div className="stepper-step-content ptn-flex ptn-flex-col ptn-items-start gap-1">
              {title}
              {description}
            </div>
          </li>
      ...
      return (
        <>
          <li
            className={cn([
              "stepper-step group peer ptn-relative ptn-flex ptn-items-center ptn-gap-2",
              "data-[variant=vertical]:ptn-flex-row",
              "data-[label-orientation=vertical]:ptn-w-full",
              "data-[label-orientation=vertical]:ptn-flex-col",
              "data-[label-orientation=vertical]:ptn-justify-center",
            ])}
          ...
          >
            <Button
              ...
              className="stepper-step-indicator ptn-rounded-full"
            ...
            <div className="stepper-step-content ptn-flex ptn-flex-col ptn-items-start">
              {title}
              {description}
            </div>
          </li>
          ...
          {variant === "vertical" && (
            <div className="ptn-flex ptn-gap-4">
              {!isLast && (
                <div className="ptn-flex ptn-justify-center ptn-ps-5">
                  <StepperSeparator
              ...
              <div className="ptn-my-3 ptn-flex-1 ptn-ps-4">{panel}</div>
            </div>
          )}
        </>
      )
    ...
    StepperControls: ({ children, className, asChild, ...props }) => {
      ...
      return (
        <Comp
          className={cn("stepper-controls ptn-flex ptn-justify-end ptn-gap-4", className)}
          {...props}
        >
          {children}
        </Comp>
...
const StepperTitle = ({
  ...
  return (
    <Comp
      className={cn("stepper-step-title ptn-text-base ptn-font-medium", className)}
      {...props}
    >
      {children}
    </Comp>
...
const StepperDescription = ({
  ...
  return (
    <Comp
      className={cn(
        "stepper-step-description ptn-text-sm ptn-text-muted-foreground",
        className
      )}
      {...props}
    >
...
const CircleStepIndicator = ({
  ...
  return (
    <div
      ...
      tabIndex={-1}
      className="stepper-step-indicator ptn-relative ptn-inline-flex ptn-items-center ptn-justify-center"
    >
      <svg width={size} height={size}>
        ...
        <circle
          ...
          className="ptn-text-muted-foreground"
        />
        <circle
          cx={size / 2}
          cy={size / 2}
          ...
          className="ptn-text-primary ptn-transition-all ptn-duration-300 ptn-ease-in-out"
          transform={`rotate(-90 ${size / 2} ${size / 2})`}
        />
      </svg>
      <div className="ptn-absolute ptn-inset-0 ptn-flex ptn-items-center ptn-justify-center">
        <span className="ptn-text-sm ptn-font-medium" aria-live="polite">
...
const listVariants = cva("stepper-navigation-list ptn-flex ptn-gap-2", {
  variants: {
    variant: {
      horizontal: "ptn-flex-row ptn-items-center ptn-justify-between",
      vertical: "ptn-flex-col",
      circle: "ptn-flex-row ptn-items-center ptn-justify-between",
    },
  },
})
...
const classForSeparator = cva(
  [
    "ptn-bg-muted",
    "data-[state=completed]:ptn-bg-primary data-[disabled]:ptn-opacity-50",
    "ptn-transition-all ptn-duration-300 ptn-ease-in-out",
  ],
  {
    variants: {
      orientation: {
        horizontal: "ptn-h-0.5 ptn-flex-1",
        vertical: "ptn-h-full ptn-w-0.5",
      },
      labelOrientation: {
        vertical:
          "ptn-absolute ptn-left-[calc(50%+30px)] ptn-right-[calc(-50%+20px)] ptn-top-5 ptn-block ptn-shrink-0",
      },
    },
  }
)
